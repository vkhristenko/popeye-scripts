...
then
    ...
    do
        echo $i
        [ -d "$DATADIR/tests_esb/${i}_${SLURM_JOB_ID}" ] && rm -rf $DATADIR/tests_esb/${i}_${SLURM_JOB_ID}
        mkdir $DATADIR/tests_esb/${i}_${SLURM_JOB_ID}
        cp -r $DATADIR/run260000/*ls0010* $DATADIR/run260000/*ls0013* $DATADIR/tests_esb/${i}_${SLURM_JOB_ID}/
    done
else
    for i in 0;
    do
...
srun run_cmsRun_wrapper.sh $LOGSDIR_JOB $NUM_CORES $DATADIR/tests_esb $VARYINPUT $USEGPU

# clean up
rm -rf $DATADIR/tests_esb/*
